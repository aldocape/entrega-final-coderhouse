...
class DaoMongoDB {
    ...
    static getInstance(collection) {
        if (!DaoMongoDB.instance) {
            ...
            DaoMongoDB.connection = mongoose_1.default
                .connect(connectionString)
                .then((con) => {
                logger_1.default.info(`Database is connected to: ${con.connection.name}`);
            })
                .catch((con) => {
                logger_1.default.error('Hubo un error en el acceso a la base de datos de MongoDB');
            });
        }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (this.collection === 'message') {
                    ...
                    documents = yield this.model
                        .find({ 'user.username': query.username })
                        .exec();
                }
                else {
                    documents = yield this.model.find(query).exec();
                }
                if (documents)
                    switch (this.collection) {
